...
func TestLoggingIntegration(t *testing.T) {
	...
	if fields["code"] != "DB_SLOW" {
		t.Errorf("Expected code: DB_SLOW, got: %v", fields["code"])
	}

	if fields["category"] != "performance" {
		t.Errorf("Expected category: performance, got: %v", fields["category"])
	}
